...
def extract(fpath, skip_completed=True, dest_dir=None):
    ...
    print(audio_feature.shape[0] / FPS)
    audio_feature = audio_feature[: int(3.2 * FPS)]
    assert (audio_feature.shape[0] - int(3.2 * FPS)) == 0, f"expected output to be ~3.2s, but was {audio_feature.shape[0] / FPS}"
