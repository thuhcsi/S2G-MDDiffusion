...
class GeneratorFullModelHandAugment(torch.nn.Module):
    """
    带有手部增强的模型汇总, 加入了hand_box对应的手部区域的l1 loss
    """

    def __init__(self, kp_extractor, bg_predictor, dense_motion_network, inpainting_network, train_params, *kwargs):
        super(GeneratorFullModelHandAugment, self).__init__()
        self.kp_extractor = kp_extractor
        self.inpainting_network = inpainting_network
        self.dense_motion_network = dense_motion_network
...
class LossFull(torch.nn.Module):
    """
    用于双帧驱动计算loss
    """

    def __init__(self, train_params, common_params):
        super(LossFull, self).__init__()
        # self.kp_extractor = kp_extractor
        # self.inpainting_network = inpainting_network
        # self.dense_motion_network = dense_motion_network
