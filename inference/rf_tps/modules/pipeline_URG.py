...
class PipelineModel(torch.nn.Module):
    ...
    def __init__(self, accalerator,
                 ...
                 unet_generator, resnet_generator, gan_discriminator, 
                 optimizer_unet, optimizer_resnet, optimizer_gan,
                 scheduler_unet, scheduler_resnet, scheduler_gan,
        ...
        self.resnet_generator = resnet_generator.to(self.device)
        self.gan_discriminator = gan_discriminator.to(self.device)
        
        self.optimizer_unet = optimizer_unet
        self.optimizer_resnet = optimizer_resnet
        self.optimizer_gan = optimizer_gan
        self.optimizers = [optimizer_unet, optimizer_resnet, optimizer_gan]
        
        self.scheduler_unet = scheduler_unet
        self.scheduler_resnet = scheduler_resnet
        self.scheduler_gan = scheduler_gan
        self.schedulers = [scheduler_unet, scheduler_resnet, scheduler_gan]
    ...
    def setup(self):
        # 做好多卡准备
        self.tps_model, \
            self.unet_generator, self.resnet_generator, self.gan_discriminator,\
            self.optimizer_unet, self.optimizer_resnet, self.optimizer_gan,\
            self.scheduler_unet, self.scheduler_resnet, self.scheduler_gan,\
    ...
    def forward(self, x):
        ...
        output_images2 = self.unet_generator(torch.cat((merged_images1, 1 - masks), dim=1)) # unet生成
        merged_images2 = merged_images1 * masks + output_images2 * (1 - masks) # unet混合，mask的黑色部分用最新的，白色部分用旧的
        output_images3 = self.resnet_generator(torch.cat((merged_images2, 1 - masks), dim=1)) # resnet生成
        merged_images3 = merged_images2 * masks + output_images3 * (1 - masks) # resnet混合，mask的黑色部分用最新的，白色部分用旧的
        
        generated['unet_prediction'] = output_images2
        generated['unet_merged_prediction'] = merged_images2
        generated['resnet_prediction'] = output_images3
        generated['resnet_merged_prediction'] = merged_images3
    ...
    def backward_gan(self):
        ...
        pred_fake = self.gan_discriminator(self.generated['unet_merged_prediction'].detach())
    ...
    def backward_generator(self):
        ...
        unet_merged_prediction = self.generated['unet_merged_prediction']
        resnet_prediction = self.generated['resnet_prediction']
        resnet_merged_prediction = self.generated['resnet_merged_prediction']
        ...
        loss_unet_l1 = loss_unet_valid + 3 * loss_unet_hole + 5 * loss_unet_hand # 按照比例设计的l1 loss
            # 计算tv perc style等损失
        # real_B_feats = self.lossNet(x['driving'])
        # fake_B_feats_unet = self.lossNet(unet_prediction)
        # comp_B_feats_unet = self.lossNet(unet_merged_prediction)
        
        # loss_unet_tv = TV_loss(unet_merged_prediction*(1-mask))
        # loss_unet_style = style_loss(real_B_feats, fake_B_feats_unet) + style_loss(real_B_feats, comp_B_feats_unet)
        # loss_unet_content = perceptual_loss(real_B_feats, fake_B_feats_unet) + perceptual_loss(real_B_feats, comp_B_feats_unet)
        ...
        pred_fake = self.gan_discriminator(unet_merged_prediction)
        ...
        # resnet的损失
        loss_resnet_valid = l1_loss_mask(resnet_prediction*mask, x['driving']*mask, mask)
        loss_resnet_hole = l1_loss_mask(resnet_prediction*(1-mask), x['driving']*(1-mask), 1-mask)
        loss_resnet_hand = l1_loss_mask(resnet_prediction*hand_mask, x['driving']*hand_mask, hand_mask) # hand_mask手部是1，其他是0
        loss_resnet_l1 = loss_resnet_valid + 3 * loss_resnet_hole + 5 * loss_resnet_hand # 按照比例设计的l1 loss
            # 计算tv perc style等损失
        real_B_feats = self.lossNet(x['driving'])
        fake_B_feats_resnet = self.lossNet(resnet_prediction)
        comp_B_feats_resnet = self.lossNet(resnet_merged_prediction)
        
        loss_resnet_tv = TV_loss(resnet_merged_prediction*(1-mask))
        loss_resnet_style = style_loss(real_B_feats, fake_B_feats_resnet) + style_loss(real_B_feats, comp_B_feats_resnet)
        loss_resnet_content = perceptual_loss(real_B_feats, fake_B_feats_resnet) + perceptual_loss(real_B_feats, comp_B_feats_resnet)
        
        self.loss_values['resnet_l1'] = loss_resnet_l1
        self.loss_values['resnet_tv'] = loss_resnet_tv
        self.loss_values['resnet_style'] = loss_resnet_style
        self.loss_values['resnet_content'] = loss_resnet_content
        ...
        # else:
        loss = loss_unet_l1 + 0.1*loss_G_GAN \
            + loss_resnet_l1 + 0.05*loss_resnet_content + 120*loss_resnet_style + 0.1*loss_resnet_tv
