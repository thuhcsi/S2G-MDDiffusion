...
class TPSModel(torch.nn.Module):
    ...
    def forward(self, x, epoch):
        ...
        if self.loss_weights['warp_loss'] != 0:
            ...
            encode_map = self.inpainting_network.get_encode(x['driving'], occlusion_map) # 多卡，.module
...
class PipelineModel(torch.nn.Module):
    ...
    def __init__(self, accalerator,
                 ...
                 optimizer, optimizer_unet, optimizer_gan,
                 scheduler_optimizer, scheduler_unet, scheduler_gan,
                 train_params, *kwargs):
        ...
        self.optimizers = [optimizer, optimizer_unet, optimizer_gan]
        
        self.scheduler_optimizer = scheduler_optimizer
        self.scheduler_unet = scheduler_unet
        self.scheduler_gan = scheduler_gan
        self.schedulers = [scheduler_optimizer, scheduler_unet, scheduler_gan]
    ...
    def forward(self, x, epoch):
        loss_values, generated = self.tps_model(x, epoch)
        # 结束tps阶段，generated['prediction']是tps后的结果
        merged_images1 = generated['prediction'].clone()  # 预测的局部模糊的图像 B,3,H,W
