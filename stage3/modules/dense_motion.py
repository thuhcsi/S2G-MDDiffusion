...
class DenseMotionNetwork(nn.Module):
    ...
    def __init__(self, block_expansion, num_blocks, max_features, num_tps, num_channels, 
                 scale_factor=0.25, bg = False, multi_mask = True, kp_variance=0.01, num_kp_per_tps=5):
        ...
        self.hourglass = Hourglass(block_expansion=block_expansion, in_features=(num_channels * (num_tps+1) + num_tps*num_kp_per_tps+1),
    ...
    def create_transformations(self, source_image, kp_driving, kp_source, bg_param):
        ...
        kp_1 = kp_1.view(bs, -1, self.num_kp_per_tps, 2)
        kp_2 = kp_2.view(bs, -1, self.num_kp_per_tps, 2)
        trans = TPS(mode = 'kp', bs = bs, kp_1 = kp_1, kp_2 = kp_2) # drving的kp1是control_kp
        driving_to_source = trans.transform_frame(source_image) # (bs, 10, h=96resize后的, w, 2) 10个tps变换
        ...
        # identity_grid[0,0,:,:,0]
        # tensor([[-1.0000, -0.9789, -0.9579,  ...,  0.9579,  0.9789,  1.0000],
        # [-1.0000, -0.9789, -0.9579,  ...,  0.9579,  0.9789,  1.0000],
        # [-1.0000, -0.9789, -0.9579,  ...,  0.9579,  0.9789,  1.0000],
        # ...,
        # [-1.0000, -0.9789, -0.9579,  ...,  0.9579,  0.9789,  1.0000],
        # [-1.0000, -0.9789, -0.9579,  ...,  0.9579,  0.9789,  1.0000],
        ...
        # [-0.9789, -0.9789, -0.9789,  ..., -0.9789, -0.9789, -0.9789],
        # [-0.9579, -0.9579, -0.9579,  ..., -0.9579, -0.9579, -0.9579],
        # ...,
        # [ 0.9579,  0.9579,  0.9579,  ...,  0.9579,  0.9579,  0.9579],
        # [ 0.9789,  0.9789,  0.9789,  ...,  0.9789,  0.9789,  0.9789],
        ...
        # affine background transformation 如果是none的话就是不变换
    ...
    def dropout_softmax(self, X, P):
        ...
        X_exp = X.exp()
        X[:,1:,...] /= (1-P)
        mask_bool =(drop == 0) # mask以P的概率为True，下面对True的部分将X_exp的值mask为0
    ...
    def forward(self, source_image, kp_driving, kp_source, bg_param = None, dropout_flag=False, dropout_p = 0):
        ...
        # dense_motion中的deformed_source根据多个tps得到的多张图
