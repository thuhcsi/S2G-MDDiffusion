...
class TPS:
    ...
    def __init__(self, mode, bs, **kwargs):
        ...
        if mode == 'random':
            ...
            kp_1 = kwargs["kp_1"]   # bs,10,5,2
            kp_2 = kwargs["kp_2"]
            device = kp_1.device
            kp_type = kp_1.type()
            self.gs = kp_1.shape[1]
            n = kp_1.shape[2]
            K = torch.norm(kp_1[:,:,:, None]-kp_1[:,:, None, :], dim=4, p=2) # None表示在该维度增加一个维度1
            K = K**2
            K = K * torch.log(K+1e-9) # bs,10,5,5
            
            one1 = torch.ones(self.bs, kp_1.shape[1], kp_1.shape[2], 1).to(device).type(kp_type) # bs,10,5,1 [p,1]=[x,y,1]
            kp_1p = torch.cat([kp_1,one1], 3)   # bs,10,5,3
            
            zero = torch.zeros(self.bs, kp_1.shape[1], 3, 3).to(device).type(kp_type) # bs,10,3,3
            P = torch.cat([kp_1p, zero],2) # bs,10,8,3
            L = torch.cat([K,kp_1p.permute(0,1,3,2)],2) #  bs,10,8,5
            L = torch.cat([L,P],3)
