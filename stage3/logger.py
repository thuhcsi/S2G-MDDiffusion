...
class Logger:
    ...
    def load_cpk(checkpoint_path, inpainting_network=None, dense_motion_network =None, kp_detector=None, 
                unet_generator=None, resnet_generator=None, e2e_generator=None, gan_discriminator=None,
                optimizer=None, optimizer_unet=None, optimizer_gan=None):
        ...
        if unet_generator is not None:
            unet_generator.load_state_dict(checkpoint['unet_generator'])
        if resnet_generator is not None:
            resnet_generator.load_state_dict(checkpoint['resnet_generator'])
        if e2e_generator is not None:
            e2e_generator.load_state_dict(checkpoint['e2e_generator'])
        if gan_discriminator is not None:
            gan_discriminator.load_state_dict(checkpoint['gan_discriminator'])
        if optimizer is not None and 'optimizer' in checkpoint:
            optimizer.load_state_dict(checkpoint['optimizer'])
        if optimizer_unet is not None and 'optimizer_unet' in checkpoint:
            optimizer_unet.load_state_dict(checkpoint['optimizer_unet'])
        if optimizer_gan is not None and 'optimizer_gan' in checkpoint:
            optimizer_gan.load_state_dict(checkpoint['optimizer_gan'])
        epoch = -1
    ...
    def log_iter_scores(self, loss_names):
        ...
        loss_string = 'Epoch:'+str(self.epoch_count).zfill(self.zfill_num) + "Iter:" + str(self.iter).zfill(self.zfill_num) +')'+ loss_string
    ...
    def log_epoch_scores(self, loss_names):
        ...
        loss_string = "; ".join(["%s - %.5f" % (name, value) for name, value in zip(loss_names, loss_mean)])
        loss_string = 'Epoch total:'+ str(self.epoch_count).zfill(self.zfill_num) + ") " + loss_string + '\n'
    ...
    def log_val_scores(self, loss_names):
        loss_mean = np.array(self.val_loss_list).mean(axis=0)
        ...
        loss_string = 'Epoch:'+str(self.epoch).zfill(self.zfill_num) + " Val:" +')'+ loss_string +'\n'
    ...
    def log_val(self, losses):
        losses = collections.OrderedDict(losses.items())
        # 每个iter把loss写入iter的loss和epoch的list，每次score iter清空iter的loss，但是直到epoch跑完才清空epoch的loss
        self.val_loss_list.append(list(losses.values()))

        self.log_val_scores(['val'])
...
class Visualizer:
    ...
    def visualize(self, driving, source, out):
        ...
        # e2e_merged_prediction
        prediction = out['e2e_merged_prediction'].data.cpu().numpy()
        prediction = np.transpose(prediction, [0, 2, 3, 1])
        images.append(prediction)
        
        
        # e2e_prediction
        prediction = out['e2e_prediction'].data.cpu().numpy()
        prediction = np.transpose(prediction, [0, 2, 3, 1])
        images.append(prediction)
...
class VisualizerTest:
    ...
    def draw_image_with_kp(self, image, kp_array, tps_group_ind):
        image = np.copy(image)
        spatial_size = np.array(image.shape[:2][::-1])[np.newaxis]
        ...
        for kp_ind, kp in enumerate(kp_array):
            rr, cc = circle(kp[1], kp[0], self.kp_size, shape=image.shape[:2])
            image[rr, cc] = np.array(self.colormap(tps_group_ind / 4))[:3]
        return image

    def create_image_column_with_kp(self, images, kp, tps_group_ind):
        image_array = np.array([self.draw_image_with_kp(v, k, tps_group_ind) for v, k in zip(images, kp)])
        return self.create_image_column(image_array)
    ...
    def create_image_grid(self, *args):
        ...
        for arg in args:
            if type(arg) == tuple:
                ...
                # for tps_ind in range(arg[1].shape[1]//20):
                for tps_ind in range(4):
                    kp = arg[1][:, tps_ind*5:(tps_ind+1)*5]
                    out.append(self.create_image_column_with_kp(arg[0], kp, tps_ind))
